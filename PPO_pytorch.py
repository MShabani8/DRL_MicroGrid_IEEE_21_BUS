# PPO with Experince replay and memory batch a modified version of PPO algorithm
...
def update(actor, critic, actor_optimizer, critic_optimizer, experiences):
    ...
    old_distribution = Categorical(F.softmax(distribution.logits.detach(), dim=-1))
    old_log_probs = old_distribution.log_prob(actions)
    ratio = torch.exp(log_probs - old_log_probs)
    surrogate1 = ratio * advantage
    surrogate2 = torch.clamp(ratio, 1 - PPO_EPS, 1 + PPO_EPS) * advantage
    actor_loss = -torch.min(surrogate1, surrogate2).mean()
    
    entropy = -distribution.entropy().mean()
    
    total_actor_loss = actor_loss + LOSS_ENTROPY * entropy
...
if __name__ == '__main__':
    ...
    trainIters(actor, critic, n_iters=1500)
